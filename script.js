...
function exportToExcel() {
  ...
  ws_data.push([])
  ws_data[ws_data.length - 1].push([])
  ws_data[ws_data.length - 1][0] = { t: "n", f: `(A42+C11)`, z: "#,##0.000" };
...
function updateCitiesTable() {
  ...
  // Создаем две строки: первая для названий городов, вторая для кнопок удаления
  const rowCities = document.createElement("tr");
  const rowActions = document.createElement("tr");

  cities.forEach(city => {
    // Если не нужно удалять главный город (id = 1), пропускаем его:
    if (city.id === 1) return;
    
    // Создаем ячейку для названия города (или его id)
    const tdCity = document.createElement("td");
    tdCity.innerText = `Город ${city.id}`;
    rowCities.appendChild(tdCity);

    // Создаем ячейку с кнопкой удаления
    ...
    rowActions.appendChild(tdAction);
  });

  // Добавляем обе строки в тело таблицы
  tbody.appendChild(rowCities);
  tbody.appendChild(rowActions);
}
